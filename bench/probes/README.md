...
|---|---|---|
| `py_codec.py` | Python codec cost on the canonical frame, the preflight/library split, the worst case the resource caps permit, and the `_TOKEN` accept-set differential | `python/mmclient/protocol.py`, `python/mmclient/_preflight.py`, `docs/PENDING_AMENDMENTS.md` (p)1b/(p)7/(p)12 |
| `tsan_struct_copy_probe.cpp` | Whether a toolchain's TSan race-checks struct-copy (memcpy-shaped) accesses — i.e. WHERE the telemetry ring's memory-ordering contract is enforceable by TSan (measured: host Apple runtime blind to the shape; container g++/libtsan reports it) | `cpp/include/mm/telemetry_ring.hpp`, `cpp/tests/test_telemetry.cpp`, `docs/PENDING_AMENDMENTS.md` (s)7 |
| `ring_push_probe.cpp` | What the telemetry ring's design refusals cost or save: L1-hot push cost, the declined cached-head amortization under saturation, the notify floor vs the real parked-waiter wake, the coherence-granule calibration behind alignas(64), and the capacity/poll drop-onset threshold | `cpp/include/mm/telemetry_ring.hpp`, `docs/PENDING_AMENDMENTS.md` (s)6 |
...
Task 14 Step 1b re-homes the remaining design-time probes (ring, UDS, ctypes microbenches)
