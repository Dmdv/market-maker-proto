...
#     near the tool paths; the limitations backlog landed it.
...
#     the design's explicit first-party scope (Step 1b), never the raw
#     DB.
...
# the limitations backlog, and the cost of leaving it open was concrete: `qualifies_as_primary`
...
# instructions (the repo's canonical setup line, the design).
$(PY):
...
.PHONY: build-dev build-rel build-asan build-tsan build-all
...
.PHONY: test-dev test-rel test-asan test-tsan
...
# ----------------------------------------------------------------------------
# C++ style. The design's own command shapes (Step 1b): clang-format
...
# `make lint mypy build-rel test-rel coverage` printed four green banners and exited 2
# because the last member died. That is correct make behaviour and a misuse on the caller's part.
#
...
#                 satisfy the design's Step 2, and each sanitizer preset costs a full build here.
#
...
# benchmark matrix shape at toy sample counts. Publishable runs stay
# scripts/run_bench.sh with its defaults, per the benchmark protocol.
.PHONY: demo
demo: $(PY) build-rel warn-mm-engine ## The demo against a rel binary built just now (STACK=tuned|naive)
